...
class MessageBoxApp(wx.App):
	...
	def GetLoginDetails(self, realm, username, may_save):
		...
		wx.XRCCTRL(self._loginDialog, "Location").SetLabel(server)
		wx.XRCCTRL(self._loginDialog, "Realm").SetLabel(prompt)
		wx.XRCCTRL(self._loginDialog, "Username").SetValue(username)
		wx.XRCCTRL(self._loginDialog, "Password").SetFocus()
