...
class MyApp(wx.App):
	...
	def OnInit(self):
		...
		if os.path.exists(self.paths_file):
			paths = [x.strip() for x in open(self.paths_file, "r").readlines()]
			for p in paths:
	...
	def OnCheckout(self, evt):
		...
		# Store the location if it's not already in the path file
		paths = []
		if os.path.exists( self.paths_file ):
			paths = [x.strip() for x in open(self.paths_file, "r").readlines()]
			
		if self.remotePath not in paths:
			paths.append( self.remotePath )
			f = open( self.paths_file, "w" )
			f.write( "\n".join( paths ) )
			f.close()
