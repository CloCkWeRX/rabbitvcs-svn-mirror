...
"""
	...
	1. Store previous commit messages.
	2. Show conflicted files in dir diff
	3. Add Import function
	4. Add gnome properties integration
	5. Sort out commit status etc.
	6. Svn Properties integration - externals etc.
"""
#==============================================================================
...
class NautilusSvn(nautilus.InfoProvider, nautilus.MenuProvider, nautilus.ColumnProvider):
	...
	"""
	EMBLEMS = {
		pysvn.wc_status_kind.added : 'cvs-added',
	...
	def get_file_items(self, window, files):
		...
		items = [ ('NautilusPython::svnlog_file_item', 'SVN Log' , 'Log of %s' % file.get_name(), self.OnShowLog),
				  ...
				  ('NautilusPython::svndelete_file_item', 'SVN Delete' , 'Remove %s from the repository.'%file.get_name(), self.OnDelete),
		...
		if os.path.isdir(path):
			...
			if os.path.isdir(os.path.join(os.path.split(path)[0], ".svn")):
				...
				if st.text_status in [pysvn.wc_status_kind.conflicted]:
					items += [	('NautilusPython::svnrevert_file_item', 'SVN Revert' , 'Revert %s back to the repository version.'%file.get_name(), self.OnRevert),
								('NautilusPython::svneditconflict_file_item', 'SVN Edit Conflicts' , 'Edit the conflicts found when updating %s.'%file.get_name(), self.OnEditConflicts),
	...
	#--------------------------------------------------------------------------
	def OnEditConflicts(self, menuitem, file):
		path = gnomevfs.get_local_path_from_uri(file.get_uri())
	...
	#-------------------------------------------------------------------------- 
	def OnResolveConflicts(self, menuitem, file):
		path = gnomevfs.get_local_path_from_uri(file.get_uri())
		c = pysvn.Client()
		c.resolved(path)
	...
	#--------------------------------------------------------------------------
	def OnRevert(self, menuitem, file):
	...
	#--------------------------------------------------------------------------
	def OnShowLog(self, menuitem, file):
	...
	def OnCommit(self, menuitem, file):
	...
	#--------------------------------------------------------------------------
	def OnUpdate(self, menuitem, file):
		path = gnomevfs.get_local_path_from_uri(file.get_uri())
	...
	def OnDelete(self, menuitem, file):
		path = gnomevfs.get_local_path_from_uri(file.get_uri())
		c = pysvn.Client()
		c.remove(path)

	#--------------------------------------------------------------------------
	def OnShowDiffDir(self, menuitem, file):
		path = gnomevfs.get_local_path_from_uri(file.get_uri())
		os.spawnl(os.P_NOWAIT, "/usr/bin/python", "python", GetPath("SvnDirDiff.py"), "%s"%path)
		
	#--------------------------------------------------------------------------
	def OnRefreshStatus(self, menuitem, file):
		file.invalidate_extension_info()

	#--------------------------------------------------------------------------
	def OnProperties(self, menuitem, file):
		path = gnomevfs.get_local_path_from_uri(file.get_uri())
